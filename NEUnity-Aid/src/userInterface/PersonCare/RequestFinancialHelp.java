...
public class RequestFinancialHelp extends javax.swing.JPanel {
    ...
    private void btnSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSubmitActionPerformed
        ...
        EducationAssistTaskRequest educationAssistTaskRequest = new EducationAssistTaskRequest();
        try {
            Long amount = Long.parseLong(txtEduFunds.getText()) + Long.parseLong(txtFundsMedExp.getText()) + Long.parseLong(txtFundsLiving.getText())
            + Long.parseLong(txtFundsMisc.getText());
            educationAssistTaskRequest.setAmount(amount);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Please enter numeric amount");
            return;
        }
        educationAssistTaskRequest.setMessage("Requesting funds");
        educationAssistTaskRequest.setStatus("Initiated");
        educationAssistTaskRequest.setSender(account);
        educationAssistTaskRequest.setPersonID(person.getPersonID());
        educationAssistTaskRequest.setRemarks("Request for Finance Team");
            ...
            for (Enterprise ent : network.getEnterpriseDirectory().getEnterpriseList()) {
                for (Organization organization : ent.getOrganizationDirectory().getOrganizationList()) {
                    if (organization instanceof FinanceOrganization) {
                        ...
                        organization.getTaskQueue().getTaskRequestList().add(educationAssistTaskRequest);
                    }
                }
            }
        }
        ...
        if (org.size() > 0) {
            account.getTaskQueue().getTaskRequestList().add(educationAssistTaskRequest);
            business.getTaskQueue().getTaskRequestList().add(educationAssistTaskRequest);
        }
