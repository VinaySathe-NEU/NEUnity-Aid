 ...
 * @author RakshitaJ
 */
public class NGOCheckProcess extends javax.swing.JPanel {
    ...
    NGO ngo;
    NGOProcessTaskRequest ngoProcessTaskRequest;
    
    public NGOCheckProcess(JPanel userProcessContainer, UserAccount account, Organization organization, Enterprise enterprise, Ecosystem business, NGODirectory ngoDirectory, NGOProcessTaskRequest ngoProcessTaskRequest, NGO ngo) {
        initComponents();
        initComponents();
        ...
        this.ngo = ngo;
        this.ngoProcessTaskRequest = ngoProcessTaskRequest;
        populateWorkRequest();
        setUserDetailsField();
    ...
    public void populateWorkRequest() {
        ...
        for (TaskRequest request : supportGroupOrganization.getTaskQueue().getTaskRequestList()) {

            if (request instanceof NGOProcessTaskRequest) {
                if (request.getUserID() == ngoProcessTaskRequest.getUserID()) {
    ...
    private void btnInitiateBCGActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInitiateBCGActionPerformed
        ...
        NGOProcessTaskRequest request = (NGOProcessTaskRequest) tblInitiateBCG.getValueAt(selectedRow, 0);
        ...
        if ("Initialized BGC".equalsIgnoreCase(request.getStatus())) {
            ...
            if (receiverval.equals(account.getUsername()) && "Pending with Adoption Organization".equals(statusval)) {
                ...
                request.setName(ngo.getName());

                //populateWorkRequest();
                BackgroundCheckTaskRequest bgcreq = new BackgroundCheckTaskRequest();
                bgcreq.setMessage("Please initiate BGC");
                bgcreq.setStatus("Pending with BGC organization");
                bgcreq.setSender(account);
                bgcreq.setUserID(ngo.getUserID());
                bgcreq.setName(ngo.getName());
                ...
                if (org != null) {
                    org.getTaskQueue().getTaskRequestList().add(bgcreq);
                    account.getTaskQueue().getTaskRequestList().add(bgcreq);
                    business.getTaskQueue().getTaskRequestList().add(bgcreq);
                }

                NGOTaskStatusCheckRequest ngoTaskStatusCheckRequest = new NGOTaskStatusCheckRequest();
                ngoTaskStatusCheckRequest.setUserID(ngo.getUserID());
                ngoTaskStatusCheckRequest.setBgcStatus("Pending");
                ngoTaskStatusCheckRequest.setFinanceStatus("Pending");
                ngoTaskStatusCheckRequest.setMessage("BGC initialized");
                ngoTaskStatusCheckRequest.setName(ngo.getName());
                    ...
                    for (Enterprise ent : network.getEnterpriseDirectory().getEnterpriseList()) {
                        for (Organization organization : ent.getOrganizationDirectory().getOrganizationList()) {
                            if (organization instanceof NGOOrganization) {
                                        if(ngo.getName().equalsIgnoreCase(organization.getName())){
                                org1 = organization;
                                break;
                                        }
                            }
                ...
                if (org1 != null) {
                    org1.getTaskQueue().getTaskRequestList().add(ngoTaskStatusCheckRequest);
                    account.getTaskQueue().getTaskRequestList().add(ngoTaskStatusCheckRequest);
                    business.getTaskQueue().getTaskRequestList().add(ngoTaskStatusCheckRequest);
                }
                ...
                try {
                    ...
                    Logger.getLogger(NGOCheckProcess.class.getName()).log(Level.SEVERE, null, ex);
                }
